...
const APP_NAME = 'Cross Country Flight Planner';
const GENERATED_BY_LINE = `Generated by ${APP_NAME}`;

function buildAirportText({ label, code, airport }) {
  const fboContacts = Array.isArray(airport.fbo_contacts) && airport.fbo_contacts.length
  ...
  return [
    ...
    ...fboContacts.map((contact, index) => (
      `FBO${fboContacts.length > 1 ? ` ${index + 1}` : ''}: ${contact.name} (Ph. ${contact.phone || ''})`
    )),
    '',
    'Runways:',
    ...runways,
...
function measureAirportCard(section, width, compact = false) {
  ...
  const fboContacts = Array.isArray(section.airport.fbo_contacts) && section.airport.fbo_contacts.length
    ? section.airport.fbo_contacts.filter((contact) => contact?.name || contact?.phone)
    : [];
  const fboLines = fboContacts.map((contact, index) => (
    `FBO${fboContacts.length > 1 ? ` ${index + 1}` : ''}: ${contact.name}${contact.phone ? ` (${contact.phone})` : ''}`
  )).reduce((sum, line) => (
    sum + countWrappedLines(line, maxCharsForWidth(contentWidth, style.metaFontSize))
  ), 0);
...
function drawAirportCard(pdf, section, x, topY, width, compact = false) {
  ...
  const hasFboMeta = fboContacts.length > 0;
  if (hasFboMeta) {
    y -= style.metaLineHeight;
  }
  fboContacts.forEach((contact, index) => {
    const line = `FBO${fboContacts.length > 1 ? ` ${index + 1}` : ''}: ${contact.name} (Ph. ${contact.phone || ''})`;
    y = drawWrappedText(pdf, line, contentX, y, {
      size: style.metaFontSize,
      color: '71 85 105',
      maxChars: maxCharsForWidth(contentWidth, style.metaFontSize),
      lineHeight: style.metaLineHeight,
    });
  });
    ...
    <div className="summary-draft">
      ...
      <div className="summary-grid">
        {report.airportSections.map((section) => {
          ...
          const fboContacts = Array.isArray(section.airport.fbo_contacts)
            ? section.airport.fbo_contacts.filter((contact) => contact?.name || contact?.phone)
            : [];
          ...
          return (
            <section key={`${section.label}-${section.code}`} className="summary-airport">
              ...
              {fboContacts.map((contact, index) => (
                <div key={`${section.code}-fbo-${index}`}>
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div className="summary-title-block">
            <h2>Trip Summary</h2>
            <div className="summary-generated-by">{GENERATED_BY_LINE}</div>
          </div>
          <button type="button" className="icon-btn" onClick={onClose}>
            ✕
          </button>
        ...
        <div className="summary-preview-actions">
          ...
          <div className="summary-preview-buttons">
            {reportMode === 'pdf' ? (
              ...
              <span className="summary-preview-spacer" aria-hidden="true" />
            )}
          </div>
        </div>
